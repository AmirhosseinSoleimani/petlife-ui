...
export class AdminGeographyComponent implements OnInit {
  ...
  remove(item:GeographyArea):void{if(!window.confirm(`Remove ${item.suburb} ${item.postcode}?`))return;this.api.delete<ApiResponse<unknown>>(`/admin/geography/${item.id}`).subscribe({next:()=>{this.successMessage='Geography removed.';this.load();},error:()=>this.errorMessage='Unable to remove geography. It may be in use.'});}
