...
export class AdminServiceCatalogComponent implements OnInit {
  ...
  load():void{this.isLoading=true;this.api.get<ApiResponse<ServiceCategory[]>>('/admin/service-catalog/categories').subscribe({next:r=>this.categories=r.data||[],error:()=>this.errorMessage='adminCatalog.loadCategoriesError'});this.api.get<ApiResponse<ServiceDefinition[]>>('/admin/service-catalog/definitions').subscribe({next:r=>{this.definitions=r.data||[];this.isLoading=false;},error:()=>{this.errorMessage='adminCatalog.loadDefinitionsError';this.isLoading=false;}});}
  saveCategory():void{this.isSaving=true;this.errorMessage='';const payload={...this.categoryForm,sortOrder:Number(this.categoryForm.sortOrder)||0};const req=this.editingCategoryId?this.api.put<ApiResponse<ServiceCategory>>(`/admin/service-catalog/categories/${this.editingCategoryId}`,payload):this.api.post<ApiResponse<ServiceCategory>>('/admin/service-catalog/categories',payload);req.subscribe({next:()=>{this.successMessage='adminCatalog.categorySaved';this.resetCategory();this.load();},error:()=>{this.errorMessage='adminCatalog.saveCategoryError';this.isSaving=false;}});}
  ...
  removeCategory(item:ServiceCategory):void{if(!window.confirm(`${this.i18n.translate('adminCatalog.removeConfirm')} ${item.name}?`))return;this.api.delete<ApiResponse<unknown>>(`/admin/service-catalog/categories/${item.id}`).subscribe({next:()=>this.load(),error:()=>this.errorMessage='adminCatalog.removeCategoryError'});}
  saveDefinition():void{this.isSaving=true;this.errorMessage='';const payload={...this.definitionForm,sortOrder:Number(this.definitionForm.sortOrder)||0};const req=this.editingDefinitionId?this.api.put<ApiResponse<ServiceDefinition>>(`/admin/service-catalog/definitions/${this.editingDefinitionId}`,payload):this.api.post<ApiResponse<ServiceDefinition>>('/admin/service-catalog/definitions',payload);req.subscribe({next:()=>{this.successMessage='adminCatalog.definitionSaved';this.resetDefinition();this.load();},error:()=>{this.errorMessage='adminCatalog.saveDefinitionError';this.isSaving=false;}});}
  ...
  removeDefinition(item:ServiceDefinition):void{if(!window.confirm(`${this.i18n.translate('adminCatalog.removeConfirm')} ${item.name}?`))return;this.api.delete<ApiResponse<unknown>>(`/admin/service-catalog/definitions/${item.id}`).subscribe({next:()=>this.load(),error:()=>this.errorMessage='adminCatalog.removeDefinitionError'});}
