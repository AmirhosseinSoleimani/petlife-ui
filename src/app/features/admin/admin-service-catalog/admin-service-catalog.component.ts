...
export class AdminServiceCatalogComponent implements OnInit {
  ...
  removeCategory(item:ServiceCategory):void{this.dialogService.confirm({title:'Remove service category?',message:`Remove ${item.name}?`,confirmLabel:'Remove category',tone:'danger'}).then(confirmed=>{if(!confirmed)return;this.api.delete<ApiResponse<unknown>>(`/admin/service-catalog/categories/${item.id}`).subscribe({next:()=>this.load(),error:()=>this.errorMessage='Category is seeded or in use and cannot be removed.'});});}
  ...
  removeDefinition(item:ServiceDefinition):void{this.dialogService.confirm({title:'Remove service definition?',message:`Remove ${item.name}?`,confirmLabel:'Remove definition',tone:'danger'}).then(confirmed=>{if(!confirmed)return;this.api.delete<ApiResponse<unknown>>(`/admin/service-catalog/definitions/${item.id}`).subscribe({next:()=>this.load(),error:()=>this.errorMessage='Definition is seeded or in use and cannot be removed.'});});}
