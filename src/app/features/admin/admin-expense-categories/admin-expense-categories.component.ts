...
export class AdminExpenseCategoriesComponent implements OnInit {
  ...
  remove(item:ExpenseCategory):void {if(!window.confirm(this.i18nService.translate('expenseCategories.deleteConfirm')))return;this.apiService.delete<ApiResponse<unknown>>(`/admin/expense-categories/${item.id}`).subscribe({next:()=>{this.successMessage='expenseCategories.deleted';this.load();},error:()=>this.errorMessage='expenseCategories.deleteError'});}
